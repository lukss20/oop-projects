...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		for(int i =0; i < points.length; i++) {
			...
			maxW = Math.max(maxW, body[i].x);
			maxH = Math.max(maxH, body[i].y);
		}
		width = maxW+1;
		height = maxH+1;
		skirt = new int[width];
	...
	*/
	...
	public Piece computeNextRotation() {
		...
		for(int i =0; i < temp.length; i++)
			temp[i] = new TPoint(height - body[i].y -1, body[i].x);
		// YOUR CODE HERE
		return new Piece(temp); // YOUR CODE HERE
	}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece temp = root.computeNextRotation();
		root.next = temp;
		for(int i =0; i < 4; i++){
			if(temp.equals(root)) break;
			Piece temp2 = temp.computeNextRotation();
			temp.next = temp2;
			temp = temp2;
		}
		temp.next = root;
